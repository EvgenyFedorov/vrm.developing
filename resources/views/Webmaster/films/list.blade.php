            ...
            <div class="card">
                <nav>
                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                        ...
                        <a class="nav-item nav-link active" id="nav-contact-tab" href="{{ url('/films') }}" role="tab" aria-controls="nav-contact" aria-selected="false">Кинотека</a>
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-md-6">Список Фильмов</div>
...
{{--                                    </a>--}}
                        ...
                        <div class="card-body" style="padding: 0; margin: 0;">
                            <table class="table table-hover table-bordered" style="margin: -1px 0 0 0;">
                                ...
                                @if(isset($data_films) AND count($data_films) > 0)

                                    @foreach($data_films as $data_film)

                                        <?php
                                        $class = ($data_film->status == 0) ? "default" : "table-success";
                                        ?>

                                        <tr id="tr_film_{{$data_film->id}}" class="{{$class}}">
                                            <td>{{$data_film->id}}</td>
                                            <td>{{$data_film->film_img_link}}</td>
                                            <td>{{$data_film->film_name}}</td>
                                            <td>
                                                @if(isset($data_film->order) && $data_film->order != null)
                                                    <div style="color: #ffffff;" type="submit" class="btn btn-primary">
                                                        Приобретен
                                                    </div>
                                                @else
                                                    <div style="color: #ffffff;" type="submit" class="btn btn-primary payment_film" id="{{$data_film->id}}">
                                                        {{$data_film->film_amount}}&nbsp;&#8381
                                                    </div>&nbsp;&nbsp;&nbsp;Купить
                                                @endif
                                            </td>
                                        </tr>
